  ...
  retries: number = 3
      ...
      for (let attempt = 0; attempt <= retries; attempt++) {
        ...
        if (response.status === 429) {
          ...
          monitoringService.logRequest(endpoint, 429, Date.now() - startTime, 'Rate limited');
        ...
        return data;
          ...
          monitoringService.logRequest(endpoint, 0, Date.now() - startTime, error instanceof Error ? error.message : 'Unknown error');
