/**
 * Split Screens
 * Centralized exports for all split-related screens
 */

export { default as SplitsListScreen } from './Splits/SplitsList/SplitsListScreen';
export { default as SplitDetailsScreen } from './SplitDetails/SplitDetailsScreen';
export { default as SplitPaymentScreen } from './SplitPayment/SplitPaymentScreen';
export { default as FairSplitScreen } from './FairSplit/FairSplitScreen';
export { default as DegenSplitScreen } from './DegenSplit/DegenSpinScreen';
