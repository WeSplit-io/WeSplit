...
const firebaseMultiSigService = {
  // Create a new multi-signature wallet
  async createMultiSigWallet(walletData: any) {
    try {
      const docRef = await addDoc(collection(db, 'multiSigWallets'), {
        ...walletData,
        created_at: serverTimestamp(),
        updated_at: serverTimestamp()
      });
  ...
  async getMultiSigWallet(walletId: string) {
    try {
      const docRef = doc(db, 'multiSigWallets', walletId);
  ...
  async getUserTransactions(userId: string) {
    try {
      const q = query(
        collection(db, 'multiSigTransactions'),
        where('signers', 'array-contains', userId)
      );
  ...
  async createMultiSigTransaction(transactionData: any) {
    try {
      const docRef = await addDoc(collection(db, 'multiSigTransactions'), {
        ...transactionData,
        created_at: serverTimestamp(),
        updated_at: serverTimestamp()
      });
  ...
  // Get a multi-signature transaction by ID
  async getMultiSigTransaction(transactionId: string) {
    try {
      const docRef = doc(db, 'multiSigTransactions', transactionId);
      const docSnap = await getDoc(docRef);
      ...
      console.error('Error getting multi-signature transaction:', error);
      return null;
    }
  },

  // Update a multi-signature transaction
  async updateMultiSigTransaction(transactionId: string, updates: any) {
    try {
      const docRef = doc(db, 'multiSigTransactions', transactionId);
      ...
      console.error('Error updating multi-signature transaction:', error);
      throw error;
    }
  }
};
