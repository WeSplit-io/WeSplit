...
class SolanaWalletService {
  ...
  async loadWallet(userId?: string, expectedAddress?: string): Promise<boolean> {
    try {
      ...
      if (!userId || !expectedAddress) {
        ...
        try {
          const { walletService } = await import('../root_level_services/WalletService');
          ...
          if (seedPhrase) {
            ...
            try {
              ...
              if (bip39.validateMnemonic(seedPhrase)) {
                ...
                const { walletService } = await import('../root_level_services/WalletService');
                const walletInfo = await walletService.importWallet(seedPhrase) as any;
  ...
  async loadWalletWithBalance(userId: string, expectedAddress: string, currency: 'SOL' | 'USDC'): Promise<boolean> {
    try {
      ...
      logger.info('Standard wallet loading failed, trying comprehensive recovery', { userId, expectedAddress }, 'SolanaWalletService');
      const recoveredWallet = await this.comprehensiveWalletRecovery(userId, expectedAddress);
      if (recoveredWallet) {
        this.keypair = recoveredWallet;
        logger.info('✅ Wallet recovered using comprehensive recovery method', { 
          address: recoveredWallet.publicKey.toBase58(),
          expectedAddress 
        }, 'SolanaWalletService');
        return true;
      }
  ...
  async tryMultipleDerivationPaths(seedPhrase: string, targetAddress: string): Promise<Keypair | null> {
    try {
      ...
      // Comprehensive list of Solana derivation paths to try
      const derivationPaths = [
      ...
      for (const path of derivationPaths) {
        try {
          ...
          const walletInfo = await walletService.importWallet(seedPhrase, path) as any;
  ...
  /**
   * Comprehensive wallet recovery method that tries all possible approaches
   */
  async comprehensiveWalletRecovery(userId: string, expectedAddress: string): Promise<Keypair | null> {
    try {
      logger.info('Starting comprehensive wallet recovery', { userId, expectedAddress }, 'SolanaWalletService');
      
      const { walletService } = await import('../root_level_services/WalletService');
      
      // Strategy 1: Try to get seed phrase and test all derivation paths
      const seedPhrase = await walletService.getSeedPhrase(userId);
      if (seedPhrase) {
        logger.info('Found seed phrase, trying comprehensive derivation paths', { userId }, 'SolanaWalletService');
        
        const foundWallet = await this.tryMultipleDerivationPaths(seedPhrase, expectedAddress);
        if (foundWallet) {
          logger.info('✅ Wallet recovered using seed phrase with alternative derivation path', { 
            userId, 
            expectedAddress,
            actualAddress: foundWallet.publicKey.toBase58()
          }, 'SolanaWalletService');
          return foundWallet;
        }
      }
      
      // Strategy 2: Try old format seed phrase
      const oldSeedPhrase = await (walletService as any).getOldFormatSecureData(`seed_phrase_${userId}`);
      if (oldSeedPhrase && oldSeedPhrase !== seedPhrase) {
        logger.info('Found old format seed phrase, trying derivation paths', { userId }, 'SolanaWalletService');
        
        const foundWallet = await this.tryMultipleDerivationPaths(oldSeedPhrase, expectedAddress);
        if (foundWallet) {
          logger.info('✅ Wallet recovered using old format seed phrase', { 
            userId, 
            expectedAddress,
            actualAddress: foundWallet.publicKey.toBase58()
          }, 'SolanaWalletService');
      ...
      // Strategy 3: Try to find any wallet with USDC balance and check if it matches
      logger.info('Trying to find any wallet with USDC balance', { userId, expectedAddress }, 'SolanaWalletService');
      ...
      for (const phrase of allSeedPhrases) {
        ...
        try {
          const walletInfo = await walletService.importWallet(phrase) as any;
          ...
          try {
            ...
            if (usdcBalance > 0) {
              logger.info('Found wallet with USDC balance', {
                address: tempKeypair.publicKey.toBase58(),
                usdcBalance,
                expectedAddress
              }, 'SolanaWalletService');
              
              // If this wallet has USDC and matches expected address, use it
              if (tempKeypair.publicKey.toBase58() === expectedAddress) {
                logger.info('✅ Found matching wallet with USDC balance', { 
                  userId, 
                  address: tempKeypair.publicKey.toBase58(),
                  usdcBalance
                }, 'SolanaWalletService');
            ...
            // Token account doesn't exist or other error, continue
          }
        } catch (error) {
          // Continue to next seed phrase
        }
      }
      
      // Strategy 4: Last resort - check if expected address has USDC balance
      ...
      try {
        ...
        if (usdcBalance > 0) {
          logger.warn('Expected address has USDC balance but no matching private key found', {
            expectedAddress,
            usdcBalance,
            issue: 'The wallet with USDC balance exists but we cannot find the private key for it. The user needs to import their wallet manually.'
          }, 'SolanaWalletService');
          
          // This is a critical issue - the user has funds but we can't access them
          return null;
        }
      } catch (error) {
        // Expected address has no USDC token account
        logger.info('Expected address has no USDC token account', { expectedAddress }, 'SolanaWalletService');
      }
      
      logger.error('Comprehensive wallet recovery failed - no matching wallet found', { 
        userId, 
        expectedAddress 
      }, 'SolanaWalletService');
      ...
      logger.error('Comprehensive wallet recovery failed', error, 'SolanaWalletService');
      return null;
    }
  }
...
export const solanaWalletService = {
  get instance() {
    console.log('🔍 solanaWalletService: Instance getter called, _solanaWalletService:', !!_solanaWalletService);
    if (!_solanaWalletService) {
      console.log('🔍 solanaWalletService: Creating new instance...');
      try {
        _solanaWalletService = new SolanaWalletService();
        console.log('🔍 solanaWalletService: Instance created successfully, has createWalletFromMnemonic:', typeof _solanaWalletService.createWalletFromMnemonic);
      } catch (error) {
        console.error('🔍 solanaWalletService: Failed to create instance:', error);
        throw error;
      }
    } else {
      console.log('🔍 solanaWalletService: Returning existing instance, has createWalletFromMnemonic:', typeof _solanaWalletService.createWalletFromMnemonic);
    }
